...
ROOT_DIR = os.path.abspath("C:/Users/Administrator/Desktop/python/swi")

# Import Mask RCNN
sys.path.append(ROOT_DIR)  # To find local version of the library

# Root directory of the models
ROOT_DIR = os.path.abspath("./taco_10_3_model")
...
class_names = ['알수없음', '플라스틱병', '병뚜껑', '알루미늄캔', '담배꽁초', '컵', '냄비뚜껑', '일반쓰레기', '비닐봉지', '캔뚜껑', '빨대']
global file_name
global class_label
...
def display_instances(image, boxes, masks, class_ids, class_names,
    ...
    # Get Max Score and idx
    max_square = None
    max_idx = None
    for idx, square in enumerate(boxes):
        y1, x1, y2, x2 = square
        if max_square is None or (x2 - x1) * (y2 - y1) > max_square:
            max_square = (x2 - x1) * (y2 - y1)
            max_idx = idx

    color = colors[max_idx]
    score = scores[max_idx] if scores is not None else None

    # Bounding box
    if not np.any(boxes[max_idx]):
        # Skip this instance. Has no bbox. Likely lost in image cropping.
        print("No result")
        return

    y1, x1, y2, x2 = boxes[max_idx]
    if show_bbox:
...
def image_segmentation():
    ...
    try:
        ...
        response['err'] = 'could not open image'
        return json.dumps(response, indent=4, ensure_ascii=False)

    # run prediction function

    detect_and_color_splash(model, img)
    ...
    try:
        ...
        response['err'] = 'could not analysis image'
        return json.dumps(response, indent=4, ensure_ascii=False)

    print(time.time() - start)
    global class_label
    response['seg'] = encoded_image
    response['class'] = class_label
    response['message'] = file_name
    return json.dumps(response, indent=4, ensure_ascii=False)


if __name__ == '__main__':
    weights_path = "./taco_10_3_model/mask_rcnn_taco_0100.h5"
    log_path = "./logs/"
